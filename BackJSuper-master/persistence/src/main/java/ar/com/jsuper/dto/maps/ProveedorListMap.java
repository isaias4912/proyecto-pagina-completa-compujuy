 ...
 */
@Component
public class ProveedorListMap extends CustomMapper<Proveedores, ProveedoresListDTO> {

	@Override
	public void mapAtoB(Proveedores a, ProveedoresListDTO b, MappingContext context) {
		if (a.getEntidad().getTipo().equals(TipoEntidad.EMPRESA)) {
			try {
				b.setDescripcion(a.getEntidad().getEmpresa().getRazonSocial() + " - " + a.getTipoDocProveedor().toString() + " " + a.getNroDocProveedor());
		...
		if (a.getEntidad().getTipo().equals(TipoEntidad.PERSONA)) {
			try {
				b.setDescripcion(a.getEntidad().getPersona().getApellido() + " " + a.getEntidad().getPersona().getNombre() + " - " + a.getTipoDocProveedor().toString() + " " + a.getNroDocProveedor());
